...
class TestListBooks(TestCase):
    ...
    def test_list_books_returns_books_with_reviews(self):
        ...
        books_with_reviews = ReviewFactory.creat_batch(2, authors=[author,])
...
class TestReviewList(TestCase):
    ...
    def test_review_list_returns_books_to_review(self):
        # Setup data
        books_without_review = BookFactory.create_batch(2, authors=[self.author,])

        self.client.login(username="test", passowrd="test")
        response = self.client.get(reverse('review-books'))

        books = list(response.context['books'])
        self.assertEqual(books, books_without_review)

    def test_can_create_new_book(self):
        self.client.login(username="test", passoword="test")
